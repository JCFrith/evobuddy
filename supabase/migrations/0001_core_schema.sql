...
comment on column public.profiles.auth_alias is
...
-- ---------------------------------------------------------------------
create table if not exists public.care_interactions (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists care_interactions_avatar_idx on public.care_interactions (avatar_id, created_at desc);
...
create index if not exists xp_history_avatar_idx on public.xp_history (avatar_id, created_at desc);
...
create index if not exists game_history_avatar_idx on public.game_history (avatar_id, played_at desc);
...
create index if not exists login_attempts_lookup_idx
  on public.login_attempts (nickname_normalized, attempted_at desc);
...
create index if not exists recovery_attempts_lookup_idx
  on public.recovery_attempts (nickname_normalized, attempted_at desc);
